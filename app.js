...
__name(commitTransaction, "commitTransaction");
...
__name(sortPendingJobs, "sortPendingJobs");
...
__name(getElementDepth, "getElementDepth");
var reactive;
var effect;
var release;
var raw;
...
function setReactivityEngine(engine) {
  ...
  effect = /* @__PURE__ */ __name((callback) => engine.effect(callback, { scheduler: /* @__PURE__ */ __name((task) => {
    if (shouldSchedule) {
      scheduler(task);
    } else {
      task();
...
function elementBoundEffect(el) {
  ...
  let wrappedEffect = /* @__PURE__ */ __name((callback, options) => {
...
__name(cleanupElement, "cleanupElement");
...
__name(flushPendingMutations, "flushPendingMutations");
...
function onMutate(mutations) {
  ...
  for (let i = 0; i < mutations.length; i++) {
    ...
    if (mutations[i].type === "attributes") {
      ...
      let add = /* @__PURE__ */ __name(() => {
        if (!addedAttributes.has(el))
          addedAttributes.set(el, []);
        addedAttributes.get(el).push({ name, value: el.getAttribute(name) });
      }, "add");
      let remove2 = /* @__PURE__ */ __name(() => {
...
__name(keyInPrototypeChain, "keyInPrototypeChain");
var mergeProxyTrap = {
  ownKeys({ objects }) {
    return Array.from(
...
__name(collapseProxies, "collapseProxies");
function initInterceptors(data2, cleanup = () => {
}) {
  let isObject3 = /* @__PURE__ */ __name((val) => typeof val === "object" && !Array.isArray(val) && val !== null, "isObject3");
...
__name(setErrorHandler, "setErrorHandler");
...
__name(normalErrorHandler, "normalErrorHandler");
...
var theEvaluatorFunction = /* @__PURE__ */ __name(() => {
}, "theEvaluatorFunction");
...
__name(setRawEvaluator, "setRawEvaluator");
...
__name(evaluateRaw, "evaluateRaw");
...
__name(normalRawEvaluator, "normalRawEvaluator");
...
__name(directiveExists, "directiveExists");
...
function getElementBoundUtilities(el) {
  let cleanups = [];
  let cleanup = /* @__PURE__ */ __name((callback) => cleanups.push(callback), "cleanup");
...
__name(interceptClone, "interceptClone");
...
__name(queueAttributesForDeferredTree, "queueAttributesForDeferredTree");
...
__name(replayQueuedAttributes, "replayQueuedAttributes");
...
__name(warnAboutMissingPlugins, "warnAboutMissingPlugins");
var tickStack = [];
var isHolding = false;
...
__name(splitClasses, "splitClasses");
function setClassesFromString(el, classString) {
  let missingClasses = /* @__PURE__ */ __name((classString2) => splitClasses(classString2).filter((i) => !el.classList.contains(i)).filter(Boolean), "missingClasses");
...
function registerTransitionsFromClassString(el, classString, stage) {
  ...
  let directiveStorageMap = {
    "enter": /* @__PURE__ */ __name((classes) => {
      el._x_transition.enter.during = classes;
    }, "enter"),
    "enter-start": /* @__PURE__ */ __name((classes) => {
      el._x_transition.enter.start = classes;
    }, "enter-start"),
    "enter-end": /* @__PURE__ */ __name((classes) => {
      el._x_transition.enter.end = classes;
    }, "enter-end"),
    "leave": /* @__PURE__ */ __name((classes) => {
      el._x_transition.leave.during = classes;
    }, "leave"),
    "leave-start": /* @__PURE__ */ __name((classes) => {
      el._x_transition.leave.start = classes;
    }, "leave-start"),
    "leave-end": /* @__PURE__ */ __name((classes) => {
...
__name(safeParseBoolean, "safeParseBoolean");
...
__name(isObjectAttr, "isObjectAttr");
...
__name(cloneIfObject, "cloneIfObject");
...
__name(hasOwnProperty2, "hasOwnProperty2");
...
__name(isReactive2, "isReactive2");
...
__name(toRaw, "toRaw");
var toReactive = /* @__PURE__ */ __name((value) => isObject(value) ? /* @__PURE__ */ reactive2(value) : value, "toReactive");
...
__name(cacheIdByNameOnElement, "cacheIdByNameOnElement");
magic("el", (el) => el);
...
directive("teleport", (el, { modifiers, expression }, { cleanup }) => {
  ...
  let placeInDom = /* @__PURE__ */ __name((clone3, target2, modifiers2) => {
...
__name(addDebounceOrThrottle, "addDebounceOrThrottle");
...
__name(isClickEvent, "isClickEvent");
...
directive("model", (el, { modifiers, expression }, { effect: effect3, cleanup }) => {
  ...
  if (isCloning) {
    ...
    let listeners = [];
    let syncValue = /* @__PURE__ */ __name((e) => setValue(getInputValue(el, modifiers, e, getValue())), "syncValue");
    ...
    if (hasBlurModifier) {
      ...
      if (el.form) {
        let form = el.form;
        let syncCallback = /* @__PURE__ */ __name(() => syncValue({ target: el }), "syncCallback");
    ...
    removeListener = /* @__PURE__ */ __name(() => listeners.forEach((remove2) => remove2()), "removeListener");
  } else {
...
var handler2 = /* @__PURE__ */ __name((el, { value, modifiers, expression, original }, { effect: effect3, cleanup }) => {
  if (!value) {
...
__name(reconcileData, "reconcileData");
interceptClone((from, to) => {
...
__name(shouldSkipRegisteringDataDuringClone, "shouldSkipRegisteringDataDuringClone");
...
__name(refreshScope, "refreshScope");
...
alpine_default.setReactivityEngine({
  ...
  effect: /* @__PURE__ */ __name((callback, options = {}) => {
    let runner;
...
// src/planner.ts
var AVERAGE_DAYS_IN_MONTH = 30.4375;
var DEFAULT_ACTIVE_WEEKDAYS = [
  "monday",
  "tuesday",
  "wednesday",
  "thursday",
...
__name(getIntervalInDays, "getIntervalInDays");
...
function daysBetween(from, to) {
  ...
  return Math.max(0, ms / (24 * 60 * 60 * 1e3));
}
__name(daysBetween, "daysBetween");
function getEffectiveLastPerformed(chore, intervalInDays, today) {
  if (chore.lastPerformed) {
    const parsed = typeof chore.lastPerformed === "string" ? new Date(chore.lastPerformed) : chore.lastPerformed;
...
__name(getEffectiveLastPerformed, "getEffectiveLastPerformed");
function getRemainingActiveDaysThisWeek(today, activeWeekdays) {
  const currentIsoDay = (today.getDay() + 6) % 7;
  let count = 0;
...
__name(getRemainingActiveDaysThisWeek, "getRemainingActiveDaysThisWeek");
function planDay(chores, options) {
  const today = normalizeDate(options?.today ?? /* @__PURE__ */ new Date());
  const todayWeekday = getWeekday(today);
  const activeWeekdaysList = options?.activeWeekdays ?? DEFAULT_ACTIVE_WEEKDAYS;
  const activeWeekdays = new Set(activeWeekdaysList);
  ...
  const evaluated = chores.map((chore) => {
    ...
    const isOverdue = urgencyRatio >= 1;
  ...
  const eligibleToday = evaluated.filter((c) => !c.deferredDueToWeekday && c.isDueThisWeek).sort((a, b) => {
    if (b.score !== a.score) {
  ...
  if (isRestDay) {
    todayTasks = [];
    backlogTasks = evaluated.filter((c) => c.isOverdue).sort((a, b) => b.score - a.score);
    upcomingTasks = evaluated.filter((c) => !c.isOverdue).sort((a, b) => a.intervalInDays - b.intervalInDays);
  } else {
    todayTasks = eligibleToday.slice(0, quotaForToday);
    const todayIds = new Set(todayTasks.map((t) => t.chore.name));
    backlogTasks = evaluated.filter((c) => !todayIds.has(c.chore.name) && c.isOverdue).sort((a, b) => b.score - a.score);
    const backlogIds = new Set(backlogTasks.map((t) => t.chore.name));
    upcomingTasks = evaluated.filter(
...
__name(planDay, "planDay");

// src/app.js
var STORAGE_KEY_CHORES = "household_chores_v1";
var STORAGE_KEY_SETTINGS = "household_settings_v1";
var SAMPLE_CHORES = [
  {
    id: "sample-1",
...
function loadChoresFromStorage() {
  try {
    const raw2 = localStorage.getItem(STORAGE_KEY_CHORES);
    if (!raw2) return null;
...
__name(loadChoresFromStorage, "loadChoresFromStorage");
function loadSettingsFromStorage() {
  try {
    const raw2 = localStorage.getItem(STORAGE_KEY_SETTINGS);
    if (!raw2) return { activeWeekdays: [...DEFAULT_ACTIVE_WEEKDAYS] };
    const parsed = JSON.parse(raw2);
...
__name(loadSettingsFromStorage, "loadSettingsFromStorage");
module_default.data("householdApp", () => ({
  chores: [],
  settings: {
  ...
  markDone(choreId) {
    ...
    if (!chore) return;
    chore.lastPerformed = (/* @__PURE__ */ new Date()).toISOString();
    this.saveChoresToStorage();
    this.addToast(`Done: "${chore.name}" \u{1F389}`);
  },
  openAddChore() {
    this.editingId = null;
    this.form = {
  ...
  openEditChore(id) {
    ...
    this.form = {
      ...
      weekdays: Array.isArray(chore.interval.weekdays) ? [...chore.interval.weekdays] : [],
  ...
  saveChore() {
    ...
    const weekdays = this.form.weekdays.length > 0 ? [...this.form.weekdays] : void 0;
    let lastPerformed = void 0;
  ...
  formatLastDone(chore) {
    ...
    const days = Math.floor(
      ((/* @__PURE__ */ new Date()).getTime() - date.getTime()) / (24 * 60 * 60 * 1e3)
    );
  ...
  restoreFromJSON(text) {
    ...
    try {
      ...
      this.saveSettingsToStorage();
      this.restoreText = "";
      this.addToast("Successfully restored data! \u{1F389}");
      this.navigateTo("today");
    } catch (err) {
  ...
  loadSampleChores() {
    ...
    this.saveChoresToStorage();
    this.addToast("Loaded 15 sample chores! \u{1F389}");
  },
  addToast(message) {
